	...
	*/
    ...
    void CrossHairExpansionWhenWalking()
    {
        ...
        {//ifnot shooting
            ...
            if (playerTransform.GetComponent<PlayerMovementScript>().maxSpeed < runningSpeed)
            { //not running
        ...
        if (bulletsIHave > 0 && bulletsInTheGun < amountOfBulletsPerLoad && !reloading/* && !aiming*/)
        {
            ...
            if (meeleAttack == false && pmS.maxSpeed != runningSpeed)
            {
                ...
                if (playerTransform.GetComponent<PlayerMovementScript>()._freakingZombiesSound)
                    playerTransform.GetComponent<PlayerMovementScript>()._freakingZombiesSound.Play();
                else
