...
public class scr_WeaponController : MonoBehaviour
{
    ...
    private void CalculateWeaponRotation()
    {
        ...
        // WeaponMovement
        targetWeaponMovementRotation.z += (isAimingIn ? settings.MovementSwayX / 3 : settings.MovementSwayX) * (settings.MovementSwayXUInverted ? -characterController.input_Movement.x : characterController.input_Movement.x);
        targetWeaponMovementRotation.x += (isAimingIn ? settings.MovementSwayY / 3 : settings.MovementSwayY) * (settings.MovementSwayYUInverted ? -characterController.input_Movement.y : characterController.input_Movement.y);
    ...
    {
        var targetPosition = LisaajousCurve(swayTime, swayAmountA, swayAmountB) / (isAimingIn ?  swayScale * 2 : swayScale);
